...
last_message = conversation.messages.where(account_id: conversation.account_id)
                           .includes(:message_translations, { attachments: [{ file_attachment: [:blob] }] })
                           .last
if last_message.blank?
  json.messages []
else
  json.messages [
    last_message.push_event_data_with_operator_translation(Current.account_user)
  ]
end
...
last_non_activity_message = conversation.messages.where(account_id: conversation.account_id)
...
json.last_non_activity_message(
  last_non_activity_message&.push_event_data_with_operator_translation(
