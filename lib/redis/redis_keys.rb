module Redis::RedisKeys
  ...
  # Comma separated optional request params an OpenAI model rejected, so they are no longer sent to it
  OPENAI_UNSUPPORTED_PARAMS = 'OPENAI_UNSUPPORTED_PARAMS::%<model>s::%<reasoning_effort>s'.freeze
  ...
  AGENT_SHIFT_REASSIGNMENT_MUTEX = 'AGENT_SHIFT_REASSIGNMENT_LOCK'.freeze
